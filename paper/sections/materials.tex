...
transporters, $70$ protein/mRNA transporters, and $200$ other transporters) and 600 non-transporter proteins for a
total of $1,380$ protein sequences. \GK{Hamid: state explicitly the discrepancy between the 780 transporters we are
using versus the 900 included in the original paper. This total appears to correspond to the ''main'' section of the
dataset described in the original paper, which suggests we threw out the validation set? We should be clear about this}.
...
for the occurrence of each amino acid residue and position combination within the sequence. \GK{Hamid, please verify this sentence}.
\end{itemize}
...
set. In the following section, the experimental design is described in detail with reference to axis of flexibility,
...
Specific values were determined through a grid search using the MCC as a cost function (Addr: 3). In the case of
SVMLight and SKLearn Probabilty scenarios, gamma and cost values were either uniform or varied across classes (Addr: 4),
whereas the implementation of the SKlearn Prediction model permitted only uniform pairs across all classes.
